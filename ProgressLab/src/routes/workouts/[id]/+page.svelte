...
<div class="head">
	...
	<div class="title-block">
		...
		<div class="muted small">
			Workout-Modus · {done} / {total} Übungen geloggt {#if data.template.description}· {data.template.description}{/if}
		</div>
	</div>
</div>
		...
		<li class:done={ex.doneNow}>
			...
			<div class="info">
				...
				<div class="muted small">
					...
					· Empfehlung: {formatRecommendation(ex.recommendation)} (RPE {ex.recommendation.rpeTarget})
				</div>
			</div>
			<div class="action">
				{#if ex.doneNow}
				...
				{:else}
					<a class="btn btn-primary small" href={`/sessions/new/${ex.id}?back=/workouts/${data.template.id}`}>
						Loggen →
					</a>
				{/if}
			</div>
...
{#if done === total}
	<div class="card done-card">
		<svg viewBox="0 0 24 24" width="40" height="40" fill="none" stroke="var(--c-success)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
			<circle cx="12" cy="12" r="11" />
			<path d="M6 12l4 4 8-9" />
		</svg>
		<h2>Workout fertig 💪</h2>
...
<style>
	...
	.ex-list li {
		...
		transition: border-color 120ms var(--ease), background 200ms var(--ease);
	}
	.ex-list li:hover {
